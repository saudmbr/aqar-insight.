...
// ── Constants ─────────────────────────────────────────────────────────────────
const PALETTE = ["#0F7BA0","#C9A84C","#34D399","#8B5CF6","#F97316","#EF4444","#06B6D4","#F59E0B","#EC4899","#14B8A6"];
        ...
        <Tabs defaultValue="overview" dir="rtl">
          ...
          <TabsContent value="overview" className="mt-5 space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              ...
              <KPICard label="إجمالي الإعلانات"    value={data?.overview.totalListings  ?? 0} sub={`+${data?.overview.newListings ?? 0} جديد خلال ${currentPeriodLabel}`} icon={Building2}  color="#C9A84C" loading={isLoading} />
              ...
              <SectionCard title="النشاط خلال الفترة المحددة" sub={`خلال ${currentPeriodLabel} — أعداد الجديد`}>
                <div className="grid grid-cols-2 gap-3 mt-3">
                  {[
                    ...
                    { label: "إعلانات جديدة",  value: data?.overview.newListings ?? 0, color: "#C9A84C" },
          ...
          <TabsContent value="users" className="mt-5 space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <KPICard label="المسوّقون المسجلون" value={data?.overview.totalMarketers ?? 0} icon={Star} color="#C9A84C" loading={isLoading} />
            </div>
          ...
          <TabsContent value="listings" className="mt-5 space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <KPICard label="مميّز (Featured)"   value={data?.listings.featuredCount ?? 0}   icon={Star}       color="#C9A84C" loading={isLoading} />
            </div>
          ...
          <TabsContent value="requests" className="mt-5 space-y-5">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <KPICard label="نشطة"            value={(data?.requests.byStatus ?? []).find(r => r.status === "active")?.count ?? 0} icon={Activity} color="#C9A84C" loading={isLoading} />
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <TabsContent value="operational" className="mt-5 space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <KPICard label="إعلانات مميّزة" value={data?.listings.featuredCount ?? 0}
                icon={Star} color="#C9A84C" loading={isLoading} />
